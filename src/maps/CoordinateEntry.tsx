...
const ElevationInput = () => {
    ...
    return (
        <>
            ...
            <NumberInput source="elevation_metres" label="Elevation (m)" />
        </>
    );
};
...
export const CoordinateInput = () => {
    ...
    const latitude_4326 = watch('latitude_4326');
    const longitude_4326 = watch('longitude_4326');

    const defaultCoordinates = [46.224413762594594, 7.359968915183943];
    const [position, setPosition] = useState(latitude_4326 && longitude_4326 ? [latitude_4326, longitude_4326] : defaultCoordinates);
    ...
    const updateLatLonFromXY = (x, y) => {
        if (isValidCoordinate(x) && isValidCoordinate(y)) {
            ...
            setValue('latitude_4326', lat, { shouldValidate: true });
            setValue('longitude_4326', lng, { shouldValidate: true });
            setPosition([lat, lng]);
        }
    };
    ...
    // Update coord_x/coord_y when latitude_4326/longitude_4326 change
    useEffect(() => {
        if (isValidCoordinate(latitude_4326) && isValidCoordinate(longitude_4326)) {
            updateXYFromLatLon(latitude_4326, longitude_4326);
        }
    }, [latitude_4326, longitude_4326]);
            ...
            <Grid container spacing={2} alignItems='center'>
                ...
                <Grid item xs={5}>
                    <Grid container spacing={2}>
                        <Grid item xs={6}>
                            <NumberInput source="coord_x" label="X Coordinate (m: SRID 2056)" validate={[required()]} disabled/>
                        </Grid>
                        <Grid item xs={6}>
                            <NumberInput source="coord_y" label="Y Coordinate (m: SRID 2056)" validate={[required()]} disabled/>
                        </Grid>
                        <Grid item xs={6}>
                            <NumberInput source="latitude_4326" label="Latitude (°)" />
                        </Grid>
                        <Grid item xs={6}>
                            <NumberInput source="longitude_4326" label="Longitude (°)" />
                        </Grid>
                        <Grid item xs={12}>
                            <ElevationInput />
                        </Grid>
